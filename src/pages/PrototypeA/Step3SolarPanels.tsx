...
interface PanelEntry extends SolarPanelFormData {
  id: string
  expanded: boolean
}

function emptyEntry(id: string): PanelEntry {
  return {
    id,
    expanded: true,
    systemType: '',
...
export default function Step3SolarPanels({ systemTypes, inverterNames, onNext, onBack, onStepClick }: Props) {
  ...
  const handleNext = () => {
    const panels: SolarPanel[] = entries.map((entry, i) => ({
      ...entry,
          ...
          >
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <Text variant="headingSm" as="span">Solar Panel {idx + 1}</Text>
              </div>
              {idx > 0 && (
                <button
